...
@ApiTags('Nomenclators - Title Brands')
@Controller('nom/title-brands')
export class TitleBrandsController {
  constructor(private readonly nomenclatorsService: NomenclatorsService) {}

  @Post()
  @AuditLog({ action: 'create', resource: 'title-brand', level: 'medium', pii: false })
  @ApiOperation({ summary: 'Create a new title brand' })
  ...
  @Get()
  @AuditLog({ action: 'read', resource: 'title-brand', level: 'low', pii: false })
  @ApiOperation({ summary: 'Get all title brands' })
  ...
  @Get(':id')
  @AuditLog({ action: 'read', resource: 'title-brand', level: 'low', pii: false })
  @ApiOperation({ summary: 'Get title brand by ID' })
  ...
  @Patch(':id')
  @AuditLog({ action: 'update', resource: 'title-brand', level: 'medium', pii: false })
  @ApiOperation({ summary: 'Update a title brand' })
  @ApiResponse({ status: HttpStatus.OK, type: NomenclatorEntity })
  async update(
    @Param('id') id: string,
  ...
  @AuditLog({ action: 'delete', resource: 'title-brand', level: 'high', pii: false })
  @ApiOperation({ summary: 'Delete a title brand' })

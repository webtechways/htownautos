// ---------------------------------------------------------------------------
// Per-tenant auction fee configuration — default values and compute helper.
// Front-end replicates the compute logic client-side; this file is the
// authoritative backend version. Keep both in sync when modifying formulas.
// ---------------------------------------------------------------------------

export type FeeValue = number | string; // string means percentage, e.g. "7.25%"

export interface BiddingFeeRow {
  min: number;
  max: number | null; // null = no upper bound ("+")
  cs: FeeValue; // clean  + secured
  cu: FeeValue; // clean  + unsecured
  ns: FeeValue; // nonClean + secured
  nu: FeeValue; // nonClean + unsecured
}

export interface FeeConfig {
  /** Default payment method when caller does not specify */
  paymentMethod: 'secured' | 'unsecured';
  /** Flat gate fee in dollars */
  gateFee: number;
  /** Flat environmental fee in dollars */
  environmentalFee: number;
  broker: {
    /** Flat broker fee floor in dollars */
    fixed: number;
    /** Broker percentage of highBid (0–100) */
    pct: number;
  };
  biddingFee: {
    rows: BiddingFeeRow[];
...
// ---------------------------------------------------------------------------
// Default Copart-style fee table
// ---------------------------------------------------------------------------
...
/**
 * Resolve a FeeValue (flat number or "N%" string) against the highBid.
 * Returns a dollar amount rounded to 2 decimal places.
 */
function resolveFeeValue(value: FeeValue, highBid: number): number {
  if (typeof value === 'number') {
    return value;
  }
  // value is a string like "7.25%"
  const pct = parseFloat(value.replace('%', ''));
  return Math.round(highBid * (pct / 100) * 100) / 100;
}

/**
 * Look up the bidding fee for a given highBid, titleType, and paymentMethod.
 * Returns 0 if no row matches (should not happen with a well-formed table).
 */
function lookupBiddingFee(
  highBid: number,
  titleType: 'clean' | 'nonClean',
  paymentMethod: 'secured' | 'unsecured',
  rows: BiddingFeeRow[],
): number {
...
/**
 * Compute the final price for an auction lot.
 *
 * finalPrice = highBid + auctionFee + brokerFee
 * auctionFee = biddingFee + gateFee + environmentalFee
 * brokerFee  = max(broker.fixed, highBid * broker.pct / 100)
 *
 * @param highBid        Winning bid amount in dollars (>= 0)
 * @param titleType      'clean' | 'nonClean'
 * @param config         FeeConfig — pass tenant.feeConfig ?? DEFAULT_FEE_CONFIG
 * @param paymentMethod  Override payment method; falls back to config.paymentMethod
 */
export function computeFinalPrice(
  ...
  const brokerFee = Math.max(
    config.broker.fixed,
    Math.round(highBid * (config.broker.pct / 100) * 100) / 100,
  );
